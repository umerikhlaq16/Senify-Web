...
import { Heart, MessageCircle, Share2, Clock, Image, Music, Home } from 'lucide-react';
...
const FamilyFeed = () => {
  ...
  return (
    <div className="min-h-screen bg-[#EEEAFB]">
      {/* Info Card */}
        <div className=" rounded-2xl p-2  mt-8">
          <div className="text-center">
            <div className="w-12 h-12 bg-purple-900 rounded-full mx-auto mb-3 flex items-center justify-center">
              <Heart className="w-6 h-6 text-white fill-current" />
            </div>
            <h3 className="text-xl font-bold text-gray-800 mb-2">Faimly Feed<br />Stay Connected with Family 
            </h3>
      ...
      <div className="max-w-8xl mx-auto px-4 py-8">
        ...
        {posts.map((post) => (
          <div key={post.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 shrink-0 w-80">
            {/* Post Header */}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 text-gray-500 text-xs mt-1">
                      <span className="font-medium text-purple-600 text-xs">{post.activity}</span>
                      <span>•</span>
            ...
            {post.type === 'video' && (
              <div className="relative bg-linear-to-br from-gray-800 to-gray-900 h-48 flex items-center justify-center">
            ...
            <div className="px-4 py-2 border-t border-gray-100 flex items-center space-x-1">
              <button 
                ...
                className={`flex-1 flex items-center justify-center space-x-1 py-2 rounded-lg font-semibold text-xs transition-all ${
                  post.liked 
              ...
              <button 
                ...
                className="flex-1 flex items-center justify-center space-x-1 py-2 rounded-lg hover:bg-gray-50 text-gray-600 font-semibold text-xs transition-all"
              >
            ...
            {showComments[post.id] && (
              <div className="px-4 py-3 bg-gray-50 border-t border-gray-100">
                  ...
                  {sampleComments[post.id]?.map((comment, idx) => (
                    <div key={idx} className="bg-white rounded-lg p-3 shadow-sm">
                      <div className="flex items-start space-x-2">
                        <div className="w-8 h-8 rounded-full bg-linear-to-br from-orange-400 to-pink-500 flex items-center justify-center text-white font-bold shrink-0 text-xs">
                          {comment.author[0]}
                        </div>
                ...
                <div className="flex space-x-2">
                  <div className="w-8 h-8 rounded-full bg-[#EEEAFB] flex items-center justify-center text-white font-bold shrink-0 text-xs">
                    Y
                  </div>
                  <input 
                    type="text"
                    placeholder="Write a comment..."
                    className="flex-1 border-2 border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-purple-400 text-xs"
                    value={commentText[post.id] || ''}
                    onChange={(e) => setCommentText({ ...commentText, [post.id]: e.target.value })}
                  />
                  <button className="bg-purple-900 text-white px-4 py-2 rounded-lg font-semibold hover:shadow-lg transition-all text-xs">
                    Post
                  </button>
                </div>
              </div>
